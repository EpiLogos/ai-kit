...
The default scope is derived from directory shape, not registration. The invocation's project is the Work member the invocation stands in: `<Central>/Work/<member>/…` names `<member>`, whether that member carries an AIKit profile marker, a ProjectCentral manifest, both, or neither. Profile discovery can legitimately resolve such a directory's project root to the world root itself (the world root carries the only marker on the walk up); the shape-derived member is what still makes that invocation a scoped project context — its own project-root node and wiki space are the context's centre, the root lineage is inherited and disclosed through the world binding, and queries lower into `: member (…)`. A member with no ProjectCentral at all scopes as `project:<member>` and discloses the absence of manifest and wiki; scoping never depends on the wiki being populated. An invocation whose cwd lies outside `Work/` (the world root, a Control location) is never guessed: it keeps world-root behaviour. An explicit `: other (…)` in the query overrides the derived default — the grammar is the surface, and an explicit scope always wins.
