...
`CentralFileMapProvider` and Central's `central.map.inspect/search/resolve` Actions.
The adapter attaches to persistent state and fetches source payloads on demand;
`rebuild` is an owner-only error. The requesting Project uses its map plus root
sources; root navigation can query participating Project maps with World provenance.
...
Central's actual prepared-index reading; preparation is the explicit native
`central.map.embeddings.prepare` operation. Semantic-only bkmr CLI output has no
...
After a generation is committed, AIKit reports its actual generated target to
`central.map.projection`. Failures at this post-commit join are explicit
reconciliation warnings, not a false claim that a committed generation was rolled
back. Central records `reported-projection`, not an observed loaded harness. A
...
adapter also refuses Central's reserved `.central/bkmr` storage, including resolved
