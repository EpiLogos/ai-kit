...
use aikit_adapters::{
    harness_disclosure::{disclose, NativeEntry, NativeObservation},
    profiles,
    tool_sources::TOOLS_PROJECTION_OWNERSHIP,
};
...
fn observe_tools(
    ...
    machine_home: &Path,
    cwd: &Path,
) -> (NativeObservation, Value, bool) {
    let Some(tools) = &profile.tools else {
        return (
            NativeObservation::default(),
            json!({"state":"not-declared"}),
            false,
        );
    };
    if tools.observe.is_empty() {
        return (
            NativeObservation::default(),
            json!({"state":"not-declared","reason":"The profile declares no native tools observation path"}),
            false,
        );
    }
    ...
    for declaration in &tools.observe {
        ...
        let reading = (|| -> std::result::Result<Option<Value>, String> {
            ...
            if !metadata.is_file() || metadata.len() > MAX_OBSERVATION_BYTES {
                return Err(
                    "The declared native configuration is not a bounded regular file".into(),
                );
            }
            let bytes = fs::read_to_string(&path).map_err(|_| {
                "The declared native configuration is not readable UTF-8".to_string()
            })?;
            if path
                .extension()
                .is_some_and(|extension| extension == "toml")
            {
                let value = toml::from_str::<toml::Value>(&bytes)
                    .map_err(|_| "The declared native TOML could not be decoded".to_string())?;
                serde_json::to_value(value)
                    .map(Some)
                    .map_err(|_| "Native TOML has an unsupported value".into())
            } else if path
                .extension()
                .is_some_and(|extension| extension == "yaml" || extension == "yml")
            {
                ...
                serde_json::from_str(&bytes)
                    .map(Some)
                    .map_err(|_| "The declared native JSON could not be decoded".into())
            }
        })();
        match reading {
            Err(reason) => {
                complete = false;
                evidence.push(json!({"path":path,"state":"unavailable","reason":reason}));
            }
            Ok(None) => evidence.push(json!({"path":path,"state":"absent"})),
            Ok(Some(document)) => {
                let mut collection = &document;
                for segment in declaration.collection.split('.') {
                    collection = collection.get(segment).unwrap_or(&Value::Null);
                }
                if !collection.is_null() && !collection.is_object() {
                    complete = false;
                    evidence.push(json!({"path":path,"state":"unavailable","reason":"The declared tools collection is not an object"}));
                    continue;
                }
                if let Some(entries) = collection.as_object() {
                    for (name, entry) in entries {
                        if names.insert(name.clone()) {
                            // The ownership marker alone is public; command paths,
                            // arguments, headers, URLs and environment values are not.
                            let owned = ["command", "args", "url"].iter().any(|key| {
                                entry.get(key).is_some_and(|value| {
                                    value.to_string().contains(TOOLS_PROJECTION_OWNERSHIP)
                                })
                            });
                            native.mcp_servers.push(NativeEntry {
                                name: name.clone(),
                                detail: owned.then(|| TOOLS_PROJECTION_OWNERSHIP.to_owned()),
                            });
                        }
                    }
                }
    ...
    (
        native,
        json!({"state":if complete {"observed"} else {"partial"},"sources":evidence}),
        complete,
    )
}

pub fn reading(service: &Service, cwd: &Path, requested: Option<&str>) -> Result<Value> {
    let machine_home = std::env::var_os("HOME").map(PathBuf::from).ok_or_else(|| {
        AikitError::new(
            "harness_disclosure.home_unavailable",
            "The native machine home is unavailable",
        )
    })?;
    let composed = crate::encounter_mcp::tool_source_entries_from_service(service)?;
    let selected = requested.map(|name| {
        crate::client::catalog_slug_for(name)
            .or_else(|| profiles::slug_for_target(&aikit_core::TargetId::new(name)))
            .unwrap_or(name)
    });
    if selected.is_some_and(|slug| profiles::for_slug(slug).is_none()) {
        return Err(AikitError::new(
            "harness_disclosure.unknown_harness",
            "The owner carries no admitted profile for this harness",
        ));
    }
    ...
    Ok(
        json!({"schema":"aikit.harness-disclosure/v1","profiles":readings,
...
mod tests {
    ...
    fn real_native_file_observation_redacts_values_and_preserves_unknown() {
        ...
        profile.tools.as_mut().unwrap().observe[0].path =
            root.join(".claude.json").to_string_lossy().into_owned();
        ...
        assert!(complete);
        assert_eq!(state["state"], "observed");
        assert_eq!(native.mcp_servers.len(), 2);
        assert!(!serde_json::to_string(&native)
            .unwrap()
        ...
        assert!(!state.to_string().contains("private-secret"));
        fs::remove_file(root.join(".claude.json")).unwrap();
        let (native, state, complete) = observe_tools(&profile, &root, &root);
        assert!(complete);
        assert!(native.mcp_servers.is_empty());
        assert_eq!(state["sources"][0]["state"], "absent");
