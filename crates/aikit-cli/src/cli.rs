...
pub enum Command {
    ...
    Session(SessionCmd),
    /// Operate durable SessionSpace semantics and the encounter owner
    /// protocol (folded from the former `aikit-session-space` companion
    /// binary; O-I #376).
    SessionSpace(SessionSpaceCmd),
...
#[derive(Debug, Args)]
pub struct SessionSpaceCmd {
    #[command(subcommand)]
    pub command: SessionSpaceCommand,
}
