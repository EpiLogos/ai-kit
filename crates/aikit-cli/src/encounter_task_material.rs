...
impl MaterialHost {
    fn check(&self) -> Result<()> {
        ...
        if !endpoint.ip().is_loopback()
            || !self.workcell_bin.is_absolute()
            || !self.workcell_bin.is_file()
            || self.required_services.len() > 32
        {
        ...
        if self
            .encounter_service
            .as_ref()
            .is_some_and(|r| !self.required_services.contains(r))
        {
            return Err(error(
                "Encounter owner service must be an explicit required service",
            ));
        }
        Ok(())
    }
    ...
    fn call(&self, operation: &str, input: Option<&Value>) -> Result<Value> {
        ...
        let mut argv = vec![
            self.workcell_bin.display().to_string(),
            "--endpoint".into(),
            self.endpoint.clone(),
            "--json".into(),
            "--state-root".into(),
        ...
        if value["ok"] != true {
            return Err(error(
                "Native Workcell returned an unsuccessful material response",
            ));
        }
        value
            .as_object_mut()
            .ok_or_else(|| error("Native material response must be an object"))?
            .remove("ok");
        Ok(value)
    }
    ...
    pub fn preflight(&self) -> Result<()> {
        if self.call("status", None)?["workcell_ref"] != json!(self.workcell_ref) {
            return Err(error(
                "Control endpoint is not the deliberately selected Workcell",
            ));
        }
        Ok(())
    }
    ...
    pub fn prepare(&self, task: &AllocatedCentralTask, subjects: Value) -> Result<MaterialBinding> {
        ...
        let mut binding = MaterialBinding {
            host: self.clone(),
            demand,
            world,
            prepared_observation: Value::Null,
        };
...
impl MaterialBinding {
    ...
    pub fn check_encounter_owner(&self) -> Result<()> {
        let Some(service) = &self.host.encounter_service else {
            return Ok(());
        };
        let bindings = self.world["binding_graph"]["bindings"]
            .as_array()
            .ok_or_else(|| error("Missing owner material bindings"))?;
        let binding = bindings
            .iter()
            .find(|b| b["port"] == "service" && b["properties"]["logical_ref"] == json!(service))
            .ok_or_else(|| error("Required encounter owner service is absent"))?;
        if binding["properties"]["pid"]
            .as_str()
    ...
    fn check_world(&self, task: &AllocatedCentralTask, world: &Value) -> Result<()> {
        ...
        if bindings.iter().any(|b| {
            b["necessity"] == "required" && (b["presence"] != "present" || b["health"] != "healthy")
        }) {
            return Err(error(
                "A required native material binding is not present and healthy",
            ));
        }
        let now_ref = &task.allocation["now_ref"];
        let stores: Vec<_> = bindings
            .iter()
            .filter(|b| b["port"] == "storage" && b["properties"]["logical_ref"] == *now_ref)
            .collect();
        if stores.len() != 1 {
            return Err(error(
                "Native task NOW must have exactly one actual storage binding",
            ));
        }
        let store = stores[0];
        let now = task.now_directory()?;
        if store["necessity"] != "required"
            || store["presence"] != "present"
            || store["health"] != "healthy"
            || store["properties"]["path"] != json!(now)
            || store["properties"]["access"] != "writable"
            || store["properties"]["sharing"] != "shared"
            || store["properties"]["object_identity"] != local_identity(&now)?
        {
            return Err(error(
                "Native storage is not the exact admitted NOW directory and required attachment",
            ));
        }
        for service in &self.host.required_services {
            ...
            if bindings
                ...
                .filter(|b| {
                    b["port"] == "service"
                        && b["properties"]["logical_ref"] == json!(service)
                        && b["necessity"] == "required"
                        && b["presence"] == "present"
                        && b["health"] == "healthy"
                })
                .count()
    ...
    pub fn validate(&self, task: &AllocatedCentralTask) -> Result<Value> {
        ...
        if current["world_ref"] != self.world["world_ref"]
            ...
            || current["provenance"]["demand_fingerprint"]
                != self.world["provenance"]["demand_fingerprint"]
        {
        ...
        for binding in current["binding_graph"]["bindings"]
            .as_array()
            .expect("checked graph")
        {
            ...
            let matches: Vec<_> = observations
                .iter()
                .filter(|o| o["logical_ref"] == binding["logical_ref"])
                .collect();
            if matches.len() != 1
                || matches[0]["state"] != "healthy"
                || matches[0]["detail"]["material_ref"] != binding["material_ref"]
                || matches[0]["detail"]["provider_ref"] != binding["provider_ref"]
            {
                return Err(error(
                    "Fresh provider observation does not confirm every required binding",
                ));
            }
            if binding["port"] == "storage"
                && (matches[0]["detail"]["path"] != binding["properties"]["path"]
                    || matches[0]["detail"]["object_identity"]
                        != binding["properties"]["object_identity"])
            {
                return Err(error(
                    "Observed NOW attachment changed path or filesystem identity",
                ));
            }
        }
        Ok(observation)
...
fn local_identity(path: &std::path::Path) -> Result<String> {
    ...
    if !metadata.is_dir() {
        return Err(error("Task storage is no longer a directory"));
    }
...
fn local_identity(_path: &std::path::Path) -> Result<String> {
    Err(error(
        "Same-object local storage correlation is unsupported on this platform",
    ))
}
