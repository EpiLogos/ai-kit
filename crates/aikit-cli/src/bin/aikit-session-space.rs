    ...
    match cli.command {
        ...
        Command::WorkingSurface { command } => match command {
            ...
            WorkingSurfaceCommand::Open { space, binding } => {
                ...
                // A Herdr open that created provider-native material returns
                // the binding carrying it; persisting that evidence is this
                // operation's separate write, through the same staged
                // mutation path as every other SessionSpace change.
