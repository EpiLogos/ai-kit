//! Native constructive actions. Pure preparation is separate from actual file
//! persistence; the desktop uses the former with Central's existing CAS write.
...
#[derive(Debug, Args)]
pub struct ConstructArgs { #[command(subcommand)] pub command: ConstructSub }
#[derive(Debug, Subcommand)]
pub enum ConstructSub {
    /// Prepare a validated native transaction from {document,request} on stdin.
    /// With --file, save atomically and read it back; document must be omitted.
    Apply { #[arg(long)] file: Option<PathBuf> },
    /// Read one full native construction and its separately addressable edges.
    Inspect { frame_ref: String, #[arg(long)] file: PathBuf },
    /// Disclose the implemented native changes; grants remain caller-owned.
    Capabilities,
}
#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct Input { #[serde(default)] document: Option<Value>, request: Request }
fn error(code: &str, detail: impl Into<String>) -> AikitError { AikitError::new(code, detail) }
fn io_error(e: impl std::fmt::Display) -> AikitError { error("knowledge.constellation_io", e.to_string()) }
fn read_bounded(path: &Path) -> Result<String> {
    let file=fs::File::open(path).map_err(io_error)?;
    let mut bytes=Vec::new();file.take(16*1024*1024+1).read_to_end(&mut bytes).map_err(io_error)?;
    if bytes.len()>16*1024*1024{return Err(error("knowledge.constellation_budget","Wiki file exceeds the explicit 16 MiB transaction budget"));}
    String::from_utf8(bytes).map_err(io_error)
}
fn input() -> Result<Input> {
    let mut data=String::new();std::io::stdin().take(16*1024*1024+1).read_to_string(&mut data).map_err(io_error)?;
    if data.len()>16*1024*1024{return Err(error("knowledge.constellation_budget","request exceeds 16 MiB"));}
    serde_json::from_str(&data).map_err(|e|error("cli.usage",format!("expected native {{document?,request}} on stdin: {e}")))
}
pub fn run(args: ConstructArgs) -> Result<Value> {
    match args.command {
        ConstructSub::Capabilities=>Ok(json!({"schema":native::ACTION,"owner":"ai-kit","action_ref":"aikit.constellation.apply",
            ...
            "revision_required":true,"operation_identity_required":true,"preparation_is_not_persistence":true,
            "ql_grammar_is_interpretive_truth":false,"automatic_publication":false})),
        ...
        ConstructSub::Apply{file}=>{
            let Input{document,request}=input()?;
            match file {
                Some(path)=>{
                    if document.is_some(){return Err(error("cli.usage","--file and a supplied document are mutually exclusive"));}
                ...
                None=>{
                    ...
                    Ok(json!({"state":"prepared","persisted":false,"applied":applied,
                        "required_return":"revision-checked native save followed by independent Wiki/search readback"}))
                }
            }
        }
    }
}
/// Same native document, not a second store. A sidecar advisory lock serialises
/// this writer; a final digest comparison detects intervening edits. Writers
/// outside this lock must use their own native owner concurrency guarantees.
fn persist(path:&Path,request:&Request)->Result<Value>{
    ...
    if !applied.idempotent {
        ...
        #[cfg(unix)] fs::File::open(parent).and_then(|dir|dir.sync_all()).map_err(io_error)?;
    }
    let after=read_bounded(path)?;
    let reading=native::inspect(&after,&request.frame_ref)?;
    if reading["frame"]["revision"]!=json!(applied.revision){return Err(error("knowledge.constellation_readback","native readback does not match the applied revision; do not retry the write automatically"));}
