...
fn read_bounded(path: &Path) -> Result<String> {
    ...
    if bytes.len() > 16 * 1024 * 1024 {
        return Err(error(
            ...
            "Wiki file exceeds the explicit 16 MiB transaction budget",
        ));
    }
...
fn input() -> Result<Input> {
    ...
    serde_json::from_str(&data).map_err(|e| {
        error(
            "cli.usage",
            format!("expected native {{document?,request}} on stdin: {e}"),
        )
    })
}
pub fn run(args: ConstructArgs) -> Result<Value> {
    match args.command {
        ConstructSub::Capabilities => Ok(
            json!({"schema":native::ACTION,"owner":"ai-kit","action_ref":"aikit.constellation.apply",
...
fn persist(path: &Path, request: &Request) -> Result<Value> {
    ...
    Ok(
        json!({"state":if applied.idempotent{"unchanged"}else{"saved"},"persisted":true,
