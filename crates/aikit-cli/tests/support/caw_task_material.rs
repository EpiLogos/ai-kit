...
impl NativeHost {
    fn new(w: &World, declare_storage: bool, managed_owner: bool) -> Self {
        let request: CentralTaskRequest =
            serde_json::from_value(w.prepare_input()["central"].clone()).unwrap();
        let task = NativeCentralPlacement::new(SystemRunner::new())
            .allocate(&request)
            .unwrap();
        let state = w.root.join("material-host");
        ...
        if declare_storage {
            fs::write(
                ...
                task.storage_declaration().unwrap().to_string(),
            )
            .unwrap();
        }
        if managed_owner {
        ...
        let mut host = Self {
            root: w.root.clone(),
            home: w.home.root().to_path_buf(),
            socket: w.socket.clone(),
            state,
            endpoint,
            child: None,
            managed_pid: None,
    ...
    fn start(&mut self) {
        self.child = Some(
            Command::new(
                std::env::var_os("AIKIT_CAW_WORKCELL_SERVICE_BIN")
                    .expect("native Workcell control service required"),
            )
        ...
        while TcpStream::connect(&self.endpoint).is_err() {
            ...
            assert!(Instant::now() < end, "native host did not listen");
    ...
    fn prepare(&mut self, w: &World, managed: bool) -> Value {
        let result = w.cli(&[
            ...
            "--request-json".into(),
            self.input(w, managed).to_string(),
        ]);
        if managed {
            self.managed_pid = Some(
                result["material"]["world"]["binding_graph"]["bindings"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .find(|b| b["port"] == "service")
                    .unwrap()["properties"]["pid"]
                    .as_str()
                    .unwrap()
                    .parse()
                    .unwrap(),
    ...
    fn workcell(&self, operation: &str, world: &Value) -> Value {
        let receipt = self.root.join("captured-material-receipt.json");
        fs::write(&receipt, world.to_string()).unwrap();
        let out = Command::new(std::env::var_os("AIKIT_CAW_WORKCELL_BIN").unwrap())
            .env("WORKCELL_CONTROL_TOKEN", TOKEN)
            .args([
                "--endpoint",
                &self.endpoint,
    ...
    fn drop(&mut self) {
        if let Some(pid) = self.managed_pid.take() {
            let _ = Command::new(env!("CARGO_BIN_EXE_aikit"))
                ...
                .arg(&self.socket)
                .arg("--request-json")
                .arg(json!({"action":"shutdown","expected_pid":pid}).to_string())
                .stdout(Stdio::null())
                .stderr(Stdio::null())
                .status();
        }
...
fn persistent_storage_reentry_and_release_govern_real_turns_without_duplicate_work() {
    ...
    let mut host = NativeHost::new(&w, true, false);
    let prepared = host.prepare(&w, false);
    let material = &prepared["material"]["world"];
    assert_eq!(prepared["ready"], true);
    assert_eq!(
        material["subjects"]["now"],
        prepared["allocation"]["allocation"]["now_ref"]
    );
    assert_eq!(material["subjects"]["agent_session"], "agent-session/task");
    assert_eq!(
        material["binding_graph"]["bindings"][0]["properties"]["path"],
    ...
    host.start();
    assert_eq!(
        host.workcell("inspect", material)["world_ref"],
        material["world_ref"]
    );
    assert_eq!(w.send("one")["data"]["duplicate"], true);
    assert_eq!(fs::read(&log).unwrap(), before);
    assert_eq!(w.send("two")["ok"], true);
    await_delivery(&w, "two");
    let before = fs::read(&log).unwrap();
    host.workcell("release", material);
    assert_eq!(w.send("three")["ok"], false);
    assert_eq!(fs::read(&log).unwrap(), before);
...
fn workcell_really_hosts_the_native_encounter_owner_and_its_protected_response() {
    ...
    let mut host = NativeHost::new(&w, true, true);
    let prepared = host.prepare(&w, true);
    assert!(host.managed_pid.is_some());
    ...
    assert_eq!(
        read["material"]["world"]["world_ref"],
        prepared["material"]["world"]["world_ref"]
    );
    assert_eq!(
        host.workcell("observe", &prepared["material"]["world"])["observations"]
            .as_array()
            .unwrap()
            .len(),
        2
...
fn absent_attachment_foreign_host_and_dropped_requirement_cannot_start_work() {
    for declared in [false, true] {
        let w = World::new(true);
        let host = NativeHost::new(&w, declared, false);
        let mut input = host.input(&w, false);
        if declared {
            input["material_host"]["workcell_ref"] = json!("workcell:wrong-owner");
        }
        let out = w.command(&[
            "encounter-task-configure".into(),
            "--agent-session".into(),
    ...
    let out = w.command(&[
        ...
        prepared["revision"].as_str().unwrap().into(),
        "--request-json".into(),
