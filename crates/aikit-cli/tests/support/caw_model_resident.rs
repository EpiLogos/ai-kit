...
fn catalogue(w: &World) -> ModelCatalogueEntry {
    let entry = ModelCatalogueEntry {
        model: r("model:controlled-caw"),
        name: "Controlled model, not commercial evidence".into(),
        description: "Native caller and identity proof".into(),
...
fn model_setup(w: &World, mode: &str, authority: bool) -> Value {
    ...
    fs::write(
        w.temp.path().join("model-provider.json"),
        provider.to_string(),
    )
    .unwrap();
    w.cli(&[
...
fn start_model(w: &mut World, key: bool) {
    ...
    command
        ...
        .env("WORKCELL_CONTROL_TOKEN", "MUST_NOT_LEAK");
    if key {
        command.env("CAW_SOURCE_API_KEY", SECRET);
    } else {
        command.env_remove("CAW_SOURCE_API_KEY");
    }
    w.child = Some(
        command
            .arg("-C")
    ...
    while !w.socket.exists() {
        assert!(Instant::now() < end);
        std::thread::sleep(Duration::from_millis(25));
    }
}
fn send(w: &World, delivery: &str) -> Value {
    let mut request = w.turn(
        "root",
        delivery,
        "Use the explicitly selected model and context",
    );
    request["turn"]["expected_binding_revision"] = json!("rev/2");
    w.request(request)
}
fn prompts(w: &World) -> usize {
    fs::read_to_string(w.temp.path().join("root.log"))
        .unwrap_or_default()
        .lines()
        .filter(|l| serde_json::from_str::<Value>(l).unwrap()["type"] == "prompt")
        .count()
}
...
fn catalogued_profileless_model_reaches_actual_resident_and_returns_with_scoped_credential() {
    ...
    assert_eq!(
        opened["data"]["model_selection"]["policy"]["model_ref"],
        "model:controlled-caw"
    );
    assert_eq!(
        opened["data"]["model_observation"]["current_model_id"],
        "controlled-model-v1"
    );
    assert_eq!(send(&w, "model-one")["ok"], true);
    assert_eq!(w.returned("root", "model-one")["data"]["phase"], "returned");
    let facts: Value =
        serde_json::from_slice(&fs::read(w.temp.path().join("root.facts.json")).unwrap()).unwrap();
    assert_eq!(facts["credential_delivered"], true);
    assert_eq!(facts["selected_context"], true);
    for field in [
        "source_credential_leaked",
        "unrelated_credential_leaked",
        "central_token_leaked",
        "workcell_token_leaked",
    ] {
        assert_eq!(facts[field], false, "{facts}");
    }
    assert_eq!(w.request(target)["ok"], true);
...
fn missing_authority_credential_and_mismatched_selection_refuse_before_provider_start() {
    for failure in [
        ...
        let mut w = World::new();
        let mut target = model_setup(&w, "normal", failure != "authority");
        match failure {
            ...
            "bounds" | "expired" => {
                ...
                if failure == "bounds" {
                    p["bounds_refs"] = json!(["bound:ungranted"]);
                } else {
                    p["expires_at_unix_ms"] = json!(1);
                }
                let bytes = serde_json::to_vec(&p).unwrap();
                fs::write(&path, &bytes).unwrap();
                let mut provider: aikit_cli::encounter_service::EncounterProvider =
                    serde_json::from_slice(
                        &fs::read(w.temp.path().join("model-provider.json")).unwrap(),
                    )
                    .unwrap();
                provider.model_policy.as_mut().unwrap().content_digest =
            ...
            "source" => {
                fs::remove_file(w.temp.path().join("model-policy.json")).unwrap();
            }
            _ => {}
        }
        start_model(&mut w, failure != "credential");
        let outcome = w.request(target);
        assert_eq!(outcome["ok"], false, "{failure}: {outcome}");
        assert!(
            !w.temp.path().join("root.log").exists(),
            "provider started for {failure}"
        );
        w.stop();
    }
}
...
fn changed_catalogue_withheld_source_and_revoked_credential_stop_subsequent_dispatch() {
    for change in ["catalogue", "withheld", "revoked"] {
        ...
        start_model(&mut w, true);
        assert_eq!(w.request(target)["ok"], true);
        assert_eq!(send(&w, "first")["ok"], true);
        assert_eq!(w.returned("root", "first")["data"]["phase"], "returned");
        match change {
            "catalogue" => {
                let mut entry = catalogue(&w);
                entry.description.push_str(" changed");
                publish_catalogue_fixture(&w, &entry);
            }
            "withheld" => {
                fs::write(w.temp.path().join(".no-agent-retrieval"), "withheld").unwrap();
            }
            "revoked" => {
                ...
                .unwrap();
                let mut state = provider.binding_state(&reference).unwrap().unwrap();
                state.revoked = true;
                CredentialBindingStore::new(&w.home).save(&state).unwrap();
            }
            _ => unreachable!(),
        }
        let outcome = send(&w, "after-change");
        assert_eq!(outcome["ok"], false, "{change}: {outcome}");
        assert_eq!(prompts(&w), 1);
        assert_eq!(w.returned("root", "first")["data"]["phase"], "returned");
        w.stop();
    }
...
fn actual_pi_model_mismatch_is_never_a_successful_selected_response() {
    for mode in ["wrong-state", "wrong-response", "drift"] {
        let mut w = World::new();
        let target = model_setup(&w, mode, true);
        start_model(&mut w, true);
        ...
        if mode == "wrong-state" {
            ...
            if mode == "wrong-response" {
                assert_eq!(returned["data"]["phase"], "failed", "{returned}");
            } else {
                assert_eq!(returned["data"]["phase"], "returned");
                assert_eq!(send(&w, "after-drift")["ok"], false);
                assert_eq!(prompts(&w), 1);
            }
        }
        w.stop();
