...
fn native_context_read_binds_exact_project_through_preview_apply_and_survives_restart() {
    ...
    assert!(
        !call(&home, &project, &["project-context"]).status.success(),
        "ordinary unbound directory cannot fabricate a context binding"
    );
    for (id, directory) in [("native-project", &project), ("native-other", &other)] {
        let result = Command::new(env!("CARGO_BIN_EXE_aikit"))
            .env("AIKIT_HOME", &home)
            .env_remove("AIKIT_CONTEXT_ID")
            .env_remove("AIKIT_ISOLATION")
            .current_dir(directory)
            .args(["--json", "project", "bind", id, "--directory"])
            .arg(directory)
            .arg("--no-default-skill-sets")
            .output()
            .unwrap();
        assert!(
    ...
    assert_eq!(
        typed.project.as_str(),
        "central/native-context",
        "native manifest identity survives directory and specification naming"
    );
    assert_eq!(
        typed
            .context
            ...
            .unwrap()
            .as_str(),
        "source:central:central/native-context:manifest"
    );
    ...
    assert_ne!(
        binding["project"], other_binding["project"],
        "native resolver keeps distinct project identities"
    );
    assert!(
        read(&home, &project, &["list"])
            .as_array()
            .unwrap()
            .is_empty(),
        "context reading creates no SessionSpace"
    );
    let space = "session-space/native-context";
    let preview = read(
        &home,
        &project,
        &["create", space, "--label", "Native context"],
    );
    assert!(
        read(&home, &project, &["list"])
            .as_array()
            .unwrap()
            .is_empty(),
        "create only stages intent"
    );
    read(
        &home,
        &project,
        &["apply", "--preview-json", &preview.to_string()],
    );
    ...
    assert!(
        read(&home, &project, &["discover", "--project", &project_ref])
            .as_array()
            .unwrap()
            .is_empty(),
        "staging does not bind membership"
    );
    let applied = read(
        &home,
        &project,
    ...
    assert_eq!(
        state, applied["after"],
        "fresh process reopens exact applied canonical state"
    );
    assert_eq!(
        read(&home, &project, &["discover", "--project", &project_ref]),
        json!([state])
    ...
    assert!(read(&home, &other, &["discover", "--project", other_ref])
        .as_array()
        .unwrap()
        .is_empty());
    let stale = call(
    ...
    assert_eq!(
        read(&home, &project, &["history", space])
            .as_array()
            .unwrap()
            .len(),
        2,
        "stale apply writes no receipt"
    );
    std::fs::write(&manifest, "{").unwrap();
    let invalid = call(&home, &project, &["project-context"]);
    assert!(
        !invalid.status.success(),
        "invalid native identity must not fall back to a directory-derived ProjectRef"
    );
