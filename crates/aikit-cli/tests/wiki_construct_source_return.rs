...
fn run(root: &Path, args: &[&str], input: Option<Value>) -> (bool, Value) {
    let mut child = Command::new(assert_cmd::cargo::cargo_bin("aikit"))
        .args(args).arg("--json").current_dir(root)
        .env("AIKIT_HOME", root.join("isolated-home"))
        .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().expect("native CLI starts");
    if let Some(input) = input {
        child.stdin.take().unwrap().write_all(input.to_string().as_bytes()).unwrap();
    } else { drop(child.stdin.take()); }
    let output = child.wait_with_output().unwrap();
    let value = serde_json::from_slice(&output.stdout).unwrap_or_else(|error| panic!("{args:?}: {error}: {} / {}", String::from_utf8_lossy(&output.stdout), String::from_utf8_lossy(&output.stderr)));
...
fn apply(root: &Path, revision: u64, operation: &str, changes: Value) -> (bool, Value) {
    ...
    run(root, &["wiki-construct", "apply", "--file", "wiki.json"], Some(json!({
        "basis_content": basis,
...
fn two_passages_from_one_source_keep_distinct_participation_and_return_without_source_mutation() {
    ...
    let sources = [provenance(first, second - 2, "An initial distinction."), provenance(second, text.len() - 1, "A different interpretation.")];
    let mut changes = vec![json!({"change":"create", "anchor_ref":"wiki:anchor:inquiry", "title":"Source inquiry",
    ...
    let members = reading["frame"]["constellations"][0]["members"].as_array().unwrap();
    assert_eq!(members.len(), 2);
    assert_eq!(members[0]["ref"], members[1]["ref"]);
    assert_ne!(members[0]["aikit.constellation-participation/v1"]["participation_ref"], members[1]["aikit.constellation-participation/v1"]["participation_ref"]);
    let selector = &members[0]["aikit.constellation-participation/v1"]["sources"][0]["aikit.techne-facet/v1"]["selector"];
    ...
    assert_eq!(reading["relations"][0]["aikit.constellation-relation/v1"]["standing"], "proposed");
    let (ok, returned) = apply(root, 1, "operation:return-expression", json!([{"change":"composition_attach", "composition":{
        "reference":"expression:source-inquiry", "revision":"3", "kind":"expression", "source":sources[0], "derivation_refs":["source:ordinary-notes"]}}]));
    assert!(ok, "{returned}");
    let (ok, reopened) = run(root, &["wiki-construct", "inspect", "wiki:frame:inquiry", "--file", "wiki.json"], None);
    assert!(ok, "{reopened}");
    assert_eq!(reopened["data"]["reading"], returned["data"]["reading"]);
    assert_eq!(reopened["data"]["reading"]["construction"]["compositions"][0]["reference"], "expression:source-inquiry");
    assert_eq!(reopened["data"]["reading"]["frame"]["constellations"][0]["returns"][0]["through_anchor_ref"], "wiki:anchor:inquiry");
    assert_eq!(fs::read_to_string(root.join("notes.md")).unwrap(), text);
    let before = fs::read(root.join("wiki.json")).unwrap();
    let (ok, refused) = apply(root, 1, "operation:late-source-edit", json!([{"change":"inquiry_set", "title":"Obsolete", "inquiry":{"question":"Old state"}}]));
    assert!(!ok, "{refused}");
