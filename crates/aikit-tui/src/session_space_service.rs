...
pub fn install_session_space_navigation_resources(
    ...
    for state in states {
        ...
        let mut descriptor =
            ResourceDescriptor::new(resource, ResourceKind::SessionSpace, label, summary);
        descriptor
            .annotations
            .insert("session-space-revision".into(), state.revision.to_string());
    ...
    fn authored_session_space_enters_common_resource_action_field_without_identity_collapse() {
        ...
        assert_eq!(
            resource
                .descriptor
                .annotations
                .get("session-space-revision"),
            Some(&"7".to_string())
        );
        assert_eq!(
            horizons_for_resource(resource),
            [AddressHorizon::H4, AddressHorizon::H5]
                .into_iter()
                .collect()
        );
        ...
        assert!(actions
            .iter()
            .all(|action| action.subject == *id.as_resource_ref()));
