...
fn concurrent_first_open_applies_each_migration_once_atomically() {
    ...
    for round in 0..8 {
            ...
            .map(|_| {
                ...
                std::thread::spawn(move || {
                    ...
                    let index = Index::open(&path)
                        .expect("concurrent native open must wait for complete schema");
                    (
                        index.schema_version().unwrap(),
                        index.applied_migrations().unwrap().len(),
                    )
                })
            })
            .collect::<Vec<_>>();
        ...
        assert_eq!(
            results[0].0 as usize, results[0].1,
            "one receipt per applied migration"
        );
    }
}
