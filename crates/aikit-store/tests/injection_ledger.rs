...
fn the_ledger_migrates_records_and_answers_the_dedup_check() {
    ...
    assert!(index
        .applied_migrations()
        .unwrap()
        .contains(&"0006-injection-ledger".to_string()));
    let ctx = "session-abc";
    assert!(!index.injection_seen(ctx, "abc123").unwrap());

    index.record_injection(ctx, "abc123").unwrap();
    assert!(
        index.injection_seen(ctx, "abc123").unwrap(),
        "recorded content is seen"
    );
    assert!(
        !index.injection_seen(ctx, "different").unwrap(),
        "other content is not"
    );
    assert!(
        !index.injection_seen("session-other", "abc123").unwrap(),
