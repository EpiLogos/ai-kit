...
impl<'a> Inbox<'a> {
    ...
    pub fn candidates(&self) -> Result<Vec<Candidate>> {
        let mut stmt = self
            .index
            .conn()
            .prepare(&format!("{CANDIDATE_COLUMNS} ORDER BY created_ns DESC, candidate_id"))
...
fn render_manifest(candidate: &Candidate, edits: &PromotionEdits, entry: &str) -> String {
    ...
    out.push_str("# Generated by `aikit promote`. Edit freely — this is now an ordinary capsule.\n");
