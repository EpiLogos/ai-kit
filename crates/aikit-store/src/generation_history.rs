...
pub fn compare_generation_worlds(
    ...
    let catalog_revision_changed =
        before_metadata.catalog_revision != after_metadata.catalog_revision;
    ...
    details.insert(
        "beforeResolutionHash".into(),
        before_metadata.resolution_hash.clone(),
    );
    details.insert(
        "afterResolutionHash".into(),
        after_metadata.resolution_hash.clone(),
    );
    ...
    details.insert(
        "availabilityChanged".into(),
        join_refs(&availability_changed),
    );
    if !target_effects_changed.is_empty() {
        details.insert(
            "targetEffectsChanged".into(),
            target_effects_changed.join(", "),
        );
    }
...
fn load_generation(
    ...
    if &metadata.generation_id != id || metadata.context_id != context.to_string() {
        return Err(AikitError::new(
            ...
            format!(
                "{} does not contain the requested Generation/Context identity",
                path.display()
            ),
        ));
    }
...
fn capsule_refs(ids: impl IntoIterator<Item = aikit_core::CapsuleId>) -> Result<Vec<ResourceRef>> {
    ids.into_iter()
