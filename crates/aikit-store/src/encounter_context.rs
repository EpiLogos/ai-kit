...
fn refusal(message: &str) -> AikitError {
    AikitError::new("encounter.context_invalid", message)
}
...
impl ContextScope {
    fn key(&self) -> Result<String> {
        if !bounded(&self.project, 4096) {
            return Err(refusal("A native project scope is required"));
        }
...
impl ContextSelection {
    pub fn validate(&self) -> Result<()> {
        if !bounded(&self.source_ref, 4096)
            || !bounded(&self.title, 4096)
            || !bounded(&self.owner, 128)
            || !bounded(&self.binding_id, 4096)
            || !bounded(&self.text, MAX_TEXT_BYTES)
            || self.text.trim().is_empty()
            || !bounded(&self.captured_at, 128)
            || self
                .source_revision
                .as_ref()
                .is_some_and(|x| !bounded(x, 4096))
            || self
                .source_project
                .as_ref()
                .is_some_and(|x| !bounded(x, 4096))
        {
            return Err(refusal(
                "Selection fields must be bounded, nonempty and NUL-free",
            ));
        }
        match &self.anchor {
            SelectionAnchor::Text { start, end } => {
                ...
                {
                    return Err(refusal(
                        "Text selection requires an exact revision and UTF-16 range",
                    ));
                }
            }
            SelectionAnchor::Observation {
                document_id,
                key,
                selector,
                role,
                ...
                if !bounded(document_id, 256)
                    || !bounded(key, 256)
                    || !bounded(selector, 4096)
                    || !bounded(role, 128)
                    || node_ref.as_ref().is_some_and(|x| !bounded(x, 4096))
                    || url.as_ref().is_some_and(|x| !bounded(x, 8192))
                {
                    return Err(refusal(
                        "An observation requires a bounded document and locator",
                    ));
                }
            }
        }
...
fn digest(value: &impl Serialize) -> Result<String> {
    Ok(format!(
        "blake3:{}",
        blake3::hash(&serde_json::to_vec(value).map_err(failure)?).to_hex()
    ))
}
...
impl PreparedItem {
    fn new(selection: ContextSelection, expression: Option<String>) -> Result<Self> {
        ...
        Ok(Self {
            ...
            canonical_expression: expression.render(),
            selection,
            expression,
        })
    }
...
pub enum ContextMutation {
    Add {
        selection: Box<ContextSelection>,
        #[serde(default)]
...
fn read_in(connection: &Connection, scope: &ContextScope) -> Result<PreparedContext> {
    let row = connection
        .query_row(
            "SELECT revision,items FROM encounter_context WHERE scope=?1",
            params![scope.key()?],
            |row| Ok((row.get::<_, u64>(0)?, row.get::<_, String>(1)?)),
        )
        .optional()
        .map_err(failure)?;
    let (revision, items) = match row {
        Some((revision, raw)) => (
            revision,
            serde_json::from_str::<Vec<PreparedItem>>(&raw).map_err(failure)?,
        ),
        None => (0, Vec::new()),
    };
    for item in &items {
        item.selection.validate()?;
        if item.id != item.selection.identity()?
            || item.canonical_expression != item.expression.render()
        {
            return Err(refusal("Corrupt prepared context"));
        }
    }
    Ok(PreparedContext {
...
fn write_in(
    ...
    if items.len() > MAX_ITEMS || encoded.len() > MAX_TOTAL_BYTES {
        return Err(refusal(
            "Prepared context exceeds 32 selections or 1 MiB; nothing was truncated",
        ));
    }
...
fn check_basis(actual: u64, expected: u64) -> Result<()> {
    if actual != expected {
        return Err(AikitError::new(
            "encounter.context_conflict",
            "Prepared context changed in another view; reread before editing",
        ));
    }
    Ok(())
}
impl EncounterStore {
    pub fn prepared_context(&self, scope: &ContextScope) -> Result<PreparedContext> {
        let connection = self.connection.lock().map_err(failure)?;
        read_in(&connection, scope)
    }
    pub fn edit_context(
        &self,
        scope: &ContextScope,
        ...
        let tx = connection
            ...
            .map_err(failure)?;
        let mut held = read_in(&tx, scope)?;
        check_basis(held.revision, basis)?;
        match mutation {
            ContextMutation::Add {
                selection,
                expression,
            } => {
                let item = PreparedItem::new(*selection, expression)?;
                if let Some(previous) = held.items.iter_mut().find(|p| p.id == item.id) {
                    *previous = item;
                } else {
                    held.items.push(item);
                }
            }
            ContextMutation::Remove { id } => {
                if !held.items.iter().any(|x| x.id == id) {
                    return Err(refusal("Selection is no longer in this context"));
                }
                held.items.retain(|x| x.id != id);
            }
        ...
        write_in(
            ...
            basis
                .checked_add(1)
                .ok_or_else(|| refusal("Revision exhausted"))?,
            &held.items,
        )?;
        let result = read_in(&tx, scope)?;
    ...
    pub fn adopt_context(
        ...
        if scope.agent_session.is_none() {
            return Err(refusal("Adoption requires a chosen session"));
        }
        let project_scope = ContextScope {
        ...
        let tx = connection
            .transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)
            .map_err(failure)?;
        let mut target = read_in(&tx, scope)?;
        let source = read_in(&tx, &project_scope)?;
        check_basis(target.revision, basis)?;
        check_basis(source.revision, project_basis)?;
        for item in source.items {
            if !target.items.iter().any(|x| x.id == item.id) {
...
pub(super) fn prepare_submission(
    connection: &Connection,
    session: &ResourceRef,
    expectation: Option<&ContextExpectation>,
) -> Result<Option<PreparedContext>> {
    let mut query = connection
        .prepare("SELECT scope FROM encounter_context WHERE session=?1 AND items!='[]'")
        .map_err(failure)?;
    let scopes = query
    ...
    let Some(expected) = expectation else {
        return if scopes.is_empty() {
            ...
            Err(AikitError::new(
                "encounter.context_review_required",
                "Review the prepared selections before sending this draft",
            ))
        };
    };
    if expected.scope.agent_session.as_ref() != Some(session) {
        return Err(refusal("Context expectation names another session"));
    }
    let expected_key = expected.scope.key()?;
    if scopes.iter().any(|key| key != &expected_key) {
        return Err(refusal(
            "This session has prepared selections in another project; resolve them explicitly",
        ));
    }
    let held = read_in(connection, &expected.scope)?;
    if held.revision != expected.revision
        || held.digest != expected.digest
        || held.items.iter().map(|x| &x.id).collect::<Vec<_>>()
            != expected.reviewed.iter().collect::<Vec<_>>()
    {
        return Err(AikitError::new(
            "encounter.context_stale",
            "Prepared selections changed after review; no provider call was made",
        ));
    }
    Ok(Some(held))
}
pub(super) fn compose(text: &str, prepared: Option<&PreparedContext>) -> Result<String> {
    let Some(context) = prepared.filter(|c| !c.items.is_empty()) else {
...
pub(super) fn clear_submitted(
    connection: &Connection,
    prepared: Option<&PreparedContext>,
) -> Result<()> {
    if let Some(c) = prepared {
        write_in(
            connection,
            &c.scope,
            c.revision
...
pub(super) fn receipts_in(connection: &Connection, session: &ResourceRef) -> Result<Vec<Value>> {
    ...
    for row in rows {
        let (cursor, raw) = row.map_err(failure)?;
        let event: Value = serde_json::from_str(&raw).map_err(failure)?;
        let c = &event["prepared_context"];
