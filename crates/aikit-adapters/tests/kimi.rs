//! Kimi CLI adapter: the harness-admission contract for Kimi CLI.
//!
//! Focused on the admission census and identity law — the parts of the contract
//! that are specific to Kimi CLI — rather than re-testing the shared projection
//! machinery already covered by the core harness-admission suite.

use aikit_adapters::clients::kimi::{KimiAdapter, PRODUCT};
...
fn target_is_distinct_and_identity_non_collapsing() {
    let adapter = KimiAdapter::new("/tmp/kimi-projection");
    assert_eq!(adapter.target().as_str(), "kimi");
...
fn admission_is_evidence_backed_and_validates() {
    ...
    assert_eq!(admission.native_version.as_deref(), Some("1.6"));
    // A model running in Kimi CLI is not the Agent identity; no actuation is claimed.
...
fn loaded_activation_overclaims_a_next_session_plan() {
    ...
    let plan = ProjectionPlan::new(
        ...
        ActivationEffect::next_session_only(
            "Kimi CLI merges AGENTS.md at session start; mid-session edits are not picked up",
        ),
    );
    let observation = HarnessActivationObservation {
        ...
        evidence_refs: vec!["native:kimi --help".to_string()],
