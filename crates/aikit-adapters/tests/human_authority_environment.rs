...
fn human_acceptance_authority_never_reaches_an_unscoped_harness() {
    ...
    if std::env::var_os(MARKER).is_none() {
        let output = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "human_acceptance_authority_never_reaches_an_unscoped_harness",
                "--nocapture",
            ])
            .env(MARKER, "controlled")
            .env("CENTRAL_NATIVE_TOKEN", "CONTROLLED_NOT_A_REAL_CREDENTIAL")
            .output()
            .unwrap();
        assert!(
    ...
    let mut child = ConnectionProcess::spawn(&argv, None).unwrap();
    assert_eq!(child.read_json().unwrap()["human_token_present"], false);
    let (_, mut reader, control) = ConnectionProcess::spawn_split_with_environment(
        &argv,
        None,
