...
use aikit_adapters::interactive_connection::{AcpStableConnectionAdapter, InteractiveAgentConnectionAdapter};
...
fn opened(result: Value) -> AcpStableConnectionAdapter {
    ...
    let open = adapter.open_session(SessionOpenRequest { mode:SessionOpenMode::Create, native_session_id:None, cwd:"/tmp".into(), additional_directories:vec![],mcp_servers:vec![],agent_session:Some(r("agent-session/test/controls")) }).unwrap();
    adapter.ingest(json!({"jsonrpc":"2.0","id":open.payload["id"],"result":result})).unwrap();
    adapter
}
#[test]
fn legacy_model_observation_does_not_invent_a_writable_config_selector() {
    let adapter = opened(json!({"sessionId":"native-test","models":{"currentModelId":"test/a","availableModels":[{"modelId":"test/a","name":"A"}]}}));
...
fn only_exact_advertised_acp_session_controls_are_writable() {
    ...
    assert!(!adapter.session_model_controls("other-native").model_selection);
    assert_eq!(adapter.set_session_model("other-native", "test/a").unwrap_err().code(), "connection.acp.model_selection_unsupported");
    assert_eq!(adapter.set_session_model("native-test", "not-advertised").unwrap_err().code(), "connection.acp.model_not_advertised");
    let command = adapter.set_session_model("native-test", "test/b").unwrap();
    assert_eq!(command.operation, "session/set_config_option");
    assert_eq!(command.payload["params"], json!({"sessionId":"native-test","configId":"model","value":"test/b"}));
}
#[test]
