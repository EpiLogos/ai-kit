//! Goose adapter: the harness-admission contract for Goose.
//!
//! Focused on the admission census and identity law — the parts of the contract
//! that are specific to Goose — rather than re-testing the shared projection
//! machinery already covered by the core harness-admission suite.

use aikit_adapters::clients::goose::{GooseAdapter, PRODUCT};
...
fn target_is_distinct_and_identity_non_collapsing() {
    let adapter = GooseAdapter::new("/tmp/goose-projection");
    assert_eq!(adapter.target().as_str(), "goose");
...
fn admission_is_evidence_backed_and_validates() {
    ...
    // A model running in Goose is not the Agent identity; no actuation claimed.
...
fn loaded_activation_overclaims_a_next_session_plan() {
    ...
    let plan = ProjectionPlan::new(
        adapter.target(),
        ActivationEffect::next_session_only(".goosehints loads at session start"),
    );
