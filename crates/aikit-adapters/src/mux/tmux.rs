...
impl Tmux<SystemRunner> {
    ...
    pub fn system() -> Self {
        Self::system_with_runner(SystemRunner::new())
    }

    /// Short read-only probes cannot hold a viewer's polling loop forever.
    pub fn bounded_system(timeout: std::time::Duration) -> Self {
        Self::system_with_runner(SystemRunner::new().with_timeout(timeout))
    }

    fn system_with_runner(runner: SystemRunner) -> Self {
        let mut adapter = Self::new(runner);
