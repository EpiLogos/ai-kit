...
impl MarkdownNode {
    fn new(kind: &str, span: Range<usize>) -> Self {
        Self { kind: kind.into(), start_byte: span.start, end_byte: span.end, text: None, attributes: BTreeMap::new(), children: Vec::new() }
    }
    fn attr(&mut self, key: &str, value: impl Into<Value>) { self.attributes.insert(key.into(), value.into()); }
}
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
fn text_of(node: &MarkdownNode) -> String {
    ...
    for child in &node.children { text.push_str(&text_of(child)); }
    text
}
fn external_destination(value: &str) -> bool {
    let lower = value.trim().to_ascii_lowercase();
    lower.starts_with("//") || lower.contains("://") || ["mailto:", "data:", "javascript:", "vbscript:", "file:", "tel:"].iter().any(|prefix| lower.starts_with(prefix))
}
    ...
    while let Some((index, current)) = chars.next() {
        ...
        if current == '#' && boundary {
            ...
            while let Some(&(offset, c)) = chars.peek() {
                if !c.is_alphanumeric() && !"_-/".contains(c) { break; }
                chars.next(); end = offset + c.len_utf8();
            }
            let tag = &text[start..end];
            if tag.chars().any(char::is_alphanumeric) { tags.push(tag.to_owned()); }
        }
...
pub fn parse_markdown_document(markdown: &str) -> MarkdownDocument {
    let options = Options::ENABLE_WIKILINKS | Options::ENABLE_TABLES | Options::ENABLE_STRIKETHROUGH
        | Options::ENABLE_TASKLISTS | Options::ENABLE_FOOTNOTES | Options::ENABLE_YAML_STYLE_METADATA_BLOCKS;
        ...
        match event {
            Event::Start(tag) => {
                ...
                match tag {
                    ...
                    Tag::Heading { level, .. } => { node.kind = "heading".into(); node.attr("level", level as u8); node.attr("id", format!("wiki-heading-{}", span.start)); }
                    Tag::BlockQuote(_) => node.kind = "blockquote".into(),
                    Tag::CodeBlock(kind) => { node.kind = "code-block".into(); if let pulldown_cmark::CodeBlockKind::Fenced(info) = kind { node.attr("info", info.to_string()); } }
                    Tag::List(start) => { node.kind = "list".into(); if let Some(start) = start { node.attr("start", start); } }
                    ...
                    Tag::Link { link_type, dest_url, title, .. } => {
                        node.kind = "link".into(); node.attr("destination", dest_url.to_string()); node.attr("title", title.to_string());
                        node.attr("syntax", if matches!(link_type, LinkType::WikiLink {..}) {"wiki"} else {"markdown"});
                    }
                    Tag::Image { dest_url, title, .. } => { node.kind = "image".into(); node.attr("destination", dest_url.to_string()); node.attr("title", title.to_string()); }
                    Tag::Table(alignments) => { node.kind = "table".into(); node.attr("alignments", Value::Array(alignments.iter().map(|align| Value::String(format!("{align:?}").to_ascii_lowercase())).collect())); }
                    Tag::TableHead => node.kind = "table-head".into(),
                    Tag::TableRow => node.kind = "table-row".into(),
                    Tag::TableCell => node.kind = "table-cell".into(),
                    Tag::FootnoteDefinition(label) => { node.kind = "footnote".into(); node.attr("label", label.to_string()); }
            ...
            Event::End(_) => {
                ...
                if node.kind == "paragraph" {
                    ...
                    if let Some(anchor) = text.split_whitespace().last().and_then(|token| token.strip_prefix('^')) {
                        if !anchor.is_empty() && anchor.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
                ...
                if node.kind == "link" || node.kind == "image" {
                    let destination = node.attributes.get("destination").and_then(Value::as_str).unwrap_or_default().to_string();
                    links.push(MarkdownLink {start_byte: node.start_byte, end_byte: node.end_byte,
                        raw_token: markdown.get(node.start_byte..node.end_byte).unwrap_or_default().to_string(),
                        external: external_destination(&destination), destination, display: text_of(&node),
                        kind: if node.kind == "image" {"image".into()} else {node.attributes.get("syntax").and_then(Value::as_str).unwrap_or("markdown").into()} });
                }
                if node.kind == "heading" { headings.push(node.clone()); }
                if node.kind == "metadata" {
                    let body = text_of(&node);
                    match serde_yaml::from_str::<BTreeMap<String, Value>>(&body) {
                        Ok(value) => properties = value,
                        Err(error) => warnings.push(format!("Markdown properties could not be read: {error}")),
                    }
                } else { stack.last_mut().expect("document root").children.push(node); }
            }
            other => {
                let mut node = MarkdownNode::new("text", span);
                match other {
                    Event::Text(text) => {
                        if !stack.iter().any(|parent| matches!(parent.kind.as_str(), "code-block" | "metadata" | "link" | "image" | "raw-html")) {
                            tags.extend(inline_tags(markdown.get(node.start_byte..node.end_byte).unwrap_or_default()));
                        }
                        node.text = Some(text.into_string());
                    }
                    Event::Code(text) => { node.kind = "code".into(); node.text = Some(text.into_string()); }
                    Event::Html(text) | Event::InlineHtml(text) => { node.kind = "raw-html".into(); node.text = Some(text.into_string()); }
                    Event::SoftBreak => node.kind = "soft-break".into(),
                    Event::HardBreak => node.kind = "hard-break".into(),
                    Event::Rule => node.kind = "rule".into(),
                    Event::TaskListMarker(checked) => { node.kind = "task".into(); node.attr("checked", checked); }
                    Event::FootnoteReference(label) => { node.kind = "footnote-reference".into(); node.text = Some(label.into_string()); }
                    Event::InlineMath(text) | Event::DisplayMath(text) => { node.kind = "math".into(); node.text = Some(text.into_string()); }
                    _ => continue,
                }
    ...
    if let Some(value) = properties.get("tags").or_else(|| properties.get("tag")) {
        match value {
            Value::String(value) => tags.extend(value.split_whitespace().map(|tag| tag.trim_start_matches('#').to_owned())),
            Value::Array(values) => tags.extend(values.iter().filter_map(Value::as_str).map(|tag| tag.trim_start_matches('#').to_owned())),
            _ => warnings.push("Markdown tags must be a string or an array of strings".into()),
        }
    }
    tags.retain(|tag| !tag.is_empty()); tags.sort(); tags.dedup();
    MarkdownDocument {version: MARKDOWN_DOCUMENT_VERSION.into(), byte_length: markdown.len(), blocks: stack.remove(0).children, links, headings, tags, properties, warnings}
}
...
pub fn authored_markdown_relations(source_ref: &SourceRef, revision: Option<&SourceRevision>, markdown: &str) -> Vec<AuthoredRelationEvidence> {
    authored_relations_from_document(source_ref, revision, &parse_markdown_document(markdown))
}

pub fn authored_relations_from_document(source_ref: &SourceRef, revision: Option<&SourceRevision>, document: &MarkdownDocument) -> Vec<AuthoredRelationEvidence> {
    document.links.iter().filter(|link| !link.external).filter_map(|link| {
        let (target, fragment) = match link.destination.split_once('#') {Some((target, fragment)) => (target, Some(fragment.to_owned())), None => (link.destination.as_str(), None)};
        let target = if target.is_empty() && fragment.is_some() {source_ref.as_str()} else {target};
        if target.trim().is_empty() { return None; }
        let mut evidence = AuthoredRelationEvidence::body_reference(source_ref.clone(), revision.cloned(), target.to_owned(), link.raw_token.clone(),
            (!link.display.is_empty()).then(|| link.display.clone()), fragment, AuthoredRelationAnchor::body(link.start_byte, link.end_byte));
        if link.kind == "image" { evidence.relation = "embeds".into(); }
        Some(evidence)
    }).collect()
}
...
mod tests {
    ...
    fn commonmark_and_wiki_share_exact_unicode_byte_occurrences() {
        ...
        for link in document.links {assert_eq!(&source[link.start_byte..link.end_byte], link.raw_token);}
    }
    #[test]
    ...
    fn escape_code_external_and_raw_html_never_become_local_semantics() {
        ...
        let relations = authored_markdown_relations(&SourceRef::parse("source:test").unwrap(), None, text);
    ...
    fn inline_tags_and_block_anchors_are_source_qualified_syntax() {
        let document = parse_markdown_document("A #reading/now and #哲学. `#code` \\#escaped [#label](other.md). ^claim-1\n");
