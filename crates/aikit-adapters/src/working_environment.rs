...
/// The observation vocabulary itself now lives in the I/O-free core so that
/// consumers which must not depend on this crate — `aikit-tui` — can still
/// read what a provider observed. Provider code keeps its established import
/// path through this re-export; the types are the same types.
pub use aikit_core::working_environment::{
