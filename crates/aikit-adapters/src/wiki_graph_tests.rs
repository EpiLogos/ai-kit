...
fn objects() -> Vec<WikiObject> {
    let member = |part: &str, role: &str, source: &str| {
        json!({"ref":source, "aikit.constellation-participation/v1":{
    ...
    let value = json!({"objects":[
        ...
        {"object":"frame","profile":"okf-wiki/v1","ref":"wiki:frame","revision":2,"member_refs":["wiki:anchor","wiki:source"],"external_refs":["PRIVATE_SOURCE"],
    ...
    aikit_core::knowledge_wiki::parse_wiki_objects(&value.to_string()).unwrap()
}
#[test]
fn repeated_source_roles_keep_native_participations_without_disclosing_private_context() {
    let value = project_graph(
        &[hit("wiki:source"), hit("wiki:anchor"), hit("wiki:frame")],
        &objects(),
        &[],
        32,
        64,
    ...
    let edge = value["edges"]
        ...
        .unwrap()
        .iter()
        .find(|e| e["reference"] == "edge:qualifies")
        .unwrap();
    assert_eq!(edge["from"], "part:one");
    assert_eq!(edge["to"], "part:two");
    assert_eq!(edge["family"], "ql-authored");
    assert_eq!(edge["standing"], "proposed");
    assert_eq!(
        value["formations"][0]["members"].as_array().unwrap().len(),
        2
    );
