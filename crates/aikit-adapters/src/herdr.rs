...
impl<R: CommandRunner> WorkingEnvironmentProvider for HerdrWorkingEnvironment<R> {
    ...
    fn focus_surface(&mut self, surface: &ResourceRef) -> Result<()> {
        ...
        // Installed Herdr 0.8.2 has no absolute pane focus: `pane focus` is
        // neighbour-relative navigation ("focus a neighboring pane"), and only
        // tab and workspace focus address their subject directly. Withhold the
        // surface-level operation rather than silently focusing something
        // else; `focus_workspace` remains the supported coarse route.
        let _ = pane;
        Err(AikitError::new(
            "herdr.surface_focus_unsupported",
            format!(
                "Herdr 0.8.2 cannot focus pane {pane} directly; `herdr pane focus` is \
                 neighbour-relative and `herdr tab/workspace focus` are coarser subjects"
            ),
        ))
    }
