...
impl SessionLane {
    ...
    pub fn model_controls(&self) -> Result<NativeModelControls> {
        ...
        if record.binding.native_session_id != self.binding.native_session_id {
            return Err(AikitError::new(
                "agent_session_host.stale_lane",
                "Model controls belong to a different native session",
            ));
        }
        if !matches!(
            ...
            return Ok(NativeModelControls::unavailable(
                "Model configuration requires an idle resident session",
            ));
        }
        let native_session_id = record.binding.native_session_id.clone();
        drop(state);
        Ok(self
            .shared
            .adapter()?
            .session_model_controls(&native_session_id))
    }
