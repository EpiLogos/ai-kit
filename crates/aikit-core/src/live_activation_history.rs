...
use crate::explain_history::{
    EvidenceProvenance, HistoryEvidence, HistoryKind, HistoryRecoverability,
};
...
mod tests {
    ...
    fn degraded_observation_remains_provider_observed_with_reason() {
        ...
        assert_eq!(evidence.authorities, vec![SourceAuthority::Observed]);
        assert_eq!(
            evidence.details.get("activationState"),
            Some(&"degraded".to_string())
        );
        assert_eq!(
            evidence.details.get("reason"),
            Some(&"pane disappeared".to_string())
        );
    }
}
