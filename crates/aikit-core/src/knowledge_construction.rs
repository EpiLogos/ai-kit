...
use crate::{
    AikitError, Result, WikiConstellation, WikiConstellationMember, WikiConstellationReturn,
    WikiEdge, WikiEdgeOrigin, WikiFrame, WikiNode, WikiObject, WikiProvenanceRef, OKF_WIKI_PROFILE,
};
...
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct WholeReference {
    pub whole_ref: ResourceRef,
    pub revision: u64,
    pub kind: String,
}
...
pub struct Participation {
    ...
    #[serde(default)]
    pub role_ref: Option<ResourceRef>,
    /// Existing provenance + ql.techne/v1 SourceSelector, not another selector grammar.
    #[serde(default)]
    pub sources: Vec<WikiProvenanceRef>,
    #[serde(default)]
    pub nested: Option<WholeReference>,
    #[serde(default)]
    pub note: String,
}
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
pub struct RelationInput {
    ...
    pub standing: String,
    #[serde(default)]
    pub evidence: Vec<WikiProvenanceRef>,
    #[serde(default)]
...
pub struct CompositionReference {
    ...
    #[serde(default)]
    pub derivation_refs: Vec<ResourceRef>,
}
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
pub struct Construction {
    ...
    #[serde(default)]
    pub frame: Option<AuthoredFrame>,
    #[serde(default)]
    pub relation_refs: Vec<ResourceRef>,
    #[serde(default)]
    pub alternatives: Vec<WholeReference>,
    #[serde(default)]
    pub variant_of: Option<WholeReference>,
    #[serde(default)]
    pub compositions: Vec<CompositionReference>,
    #[serde(default)]
...
pub enum Change {
    Create {
        ...
        inquiry: Inquiry,
        #[serde(default)]
        space_refs: Vec<ResourceRef>,
        #[serde(default)]
        frame: Option<AuthoredFrame>,
        #[serde(default)]
        variant_of: Option<WholeReference>,
    },
    InquirySet {
        title: String,
        inquiry: Inquiry,
    ...
    RoleSet {
        participation_ref: ResourceRef,
        role_ref: Option<ResourceRef>,
    },
    SourcesSet {
        participation_ref: ResourceRef,
        sources: Vec<WikiProvenanceRef>,
    },
    RelationPut {
    ...
    AlternativeAdd {
        alternative: WholeReference,
    },
    AlternativeRemove {
        whole_ref: ResourceRef,
    },
    CompositionAttach {
...
fn err(message: impl Into<String>) -> AikitError {
    AikitError::new("knowledge.constellation_refused", message)
}
...
fn text(value: &str, field: &str, max: usize) -> Result<()> {
    if value.trim().is_empty() || value.len() > max || value.contains('\0') {
        return Err(err(format!("invalid {field}")));
    }
    Ok(())
}
fn core_metadata(frame: &WikiFrame) -> Result<Construction> {
    decoded(
        frame.extensions.get(CONSTRUCTION).ok_or_else(|| {
            err("not a constructive WikiFrame; adopt it explicitly before editing")
        })?,
    )
}
fn participation(member: &WikiConstellationMember) -> Result<Participation> {
    decoded(
        member
            .extensions
            .get(PARTICIPATION)
            .ok_or_else(|| err("member has no contextual participation identity"))?,
    )
}
fn selection_sources(sources: &[WikiProvenanceRef]) -> Result<()> {
    if sources.len() > LIMIT {
        return Err(err("source budget exceeded"));
    }
    for source in sources {
        crate::resource::SourceRef::parse(source.source_ref.as_str())?;
        if matches!(
            &source.source_revision,
            Some(crate::SemanticRevision::Number(0))
        ) || matches!(&source.source_revision,Some(crate::SemanticRevision::Text(s)) if s.trim().is_empty())
        {
            return Err(err("source revision is empty"));
        }
...
fn check_reference(reference: &WholeReference, target: &ResourceRef) -> Result<()> {
    ...
    if reference.revision == 0 || &reference.whole_ref == target {
        return Err(err(
            "a nested/alternative/variant reference requires another whole at an exact revision",
        ));
    }
    if ![
        "constellation",
    ...
    .contains(&reference.kind.as_str())
    {
        return Err(err("unsupported whole kind"));
    }
    Ok(())
}
fn frame_of(doc: &WikiDocument, reference: &ResourceRef) -> Result<WikiFrame> {
    match doc.object(reference) {
        Some(WikiObject::Frame(f)) if f.constellations.len() == 1 => Ok(f.clone()),
        _ => Err(err("the native frame is absent")),
    }
}
fn member_index(frame: &WikiFrame, reference: &ResourceRef) -> Result<usize> {
    frame.constellations[0]
        .members
        .iter()
        .position(|m| participation(m).is_ok_and(|p| &p.participation_ref == reference))
        .ok_or_else(|| {
            err(format!(
                "participation {reference} is not in this construction"
            ))
        })
}
fn revise_member(
    ...
    change: impl FnOnce(&mut Participation),
) -> Result<()> {
    ...
    match doc.object(object.ref_id()) {
        ...
        _ => {
            return Err(AikitError::new(
                ...
                format!(
                    "native object {} changed or already exists",
                    object.ref_id()
                ),
            ))
        }
...
pub fn inspect(input: &str, reference: &ResourceRef) -> Result<Value> {
    ...
    let relations: Vec<_> = meta
        .relation_refs
        .iter()
        .map(|r| match doc.object(r) {
            Some(WikiObject::Edge(edge)) if edge.origin_ref.as_ref() == Some(reference) => {
                encoded(edge)
            }
            _ => Err(err(
                "relation ref is absent or does not name this construction's native edge",
            )),
        })
        .collect::<Result<_>>()?;
    Ok(
        json!({"schema":CONSTRUCTION,"frame":frame,"construction":meta,"relations":relations,
...
pub fn apply(input: &str, request: &Request) -> Result<Applied> {
    ...
    if request.schema != ACTION || request.changes.is_empty() || request.changes.len() > 256 {
        return Err(err("unsupported request schema or change budget"));
    }
    ...
    if let Some(WikiObject::Frame(frame)) = original.object(&request.frame_ref) {
        ...
        if let Some(prior) = meta.applied.get(request.operation_ref.as_str()) {
            if prior.request_digest != digest {
                return Err(err("operation identity was reused with different content"));
            }
            return Ok(Applied {
                schema: CONSTRUCTION,
    ...
    let (content, outcome) = apply_wiki_mutation(input, |doc, ledger| {
        ...
        if request.expected_revision == 0 {
            let Some(Change::Create {
                ...
                return Err(err("new construction must start with create"));
            };
            if doc.holds(&request.frame_ref) || doc.holds(anchor_ref) {
                return Err(err("construction/anchor identity already exists"));
            }
            if anchor_ref == &request.frame_ref {
                return Err(err("the anchor and frame are distinct native objects"));
            }
            meta = Construction {
                title: title.clone(),
                inquiry: inquiry.clone(),
                authored_by: request.actor_ref.clone(),
                frame: form.clone(),
                relation_refs: vec![],
                alternatives: vec![],
            ...
            frame = WikiFrame {
                ...
                ref_id: request.frame_ref.clone(),
                revision: 1,
                provenance: vec![],
                inquiry_ref: None,
                space_refs: space_refs.clone(),
                member_refs: vec![anchor_ref.clone()],
                external_refs: vec![],
            ...
            if let Some(variant) = variant_of {
                ...
                for (i, edge_ref) in base_meta.relation_refs.iter().enumerate() {
                    let Some(WikiObject::Edge(prior)) = doc.object(edge_ref) else {
                        return Err(err("variant relation is absent"));
                    };
                    let mut edge = prior.clone();
                    let mut relation: RelationInput = decoded(
                        edge.extensions
                            .get(RELATION)
                            .ok_or_else(|| err("variant relation has no participation basis"))?,
                    )?;
                    if relation.standing == "retracted" {
                        continue;
                    }
                    relation.relation_ref =
                        ResourceRef::parse(format!("{}:relation:{i}", request.frame_ref))?;
                    relation.expected_revision = None;
                    relation.from_participation_ref = remapped
                        .get(&relation.from_participation_ref)
                        .ok_or_else(|| err("variant relation endpoint is absent"))?
                        .clone();
                    relation.to_participation_ref = remapped
                        .get(&relation.to_participation_ref)
                        .ok_or_else(|| err("variant relation endpoint is absent"))?
                        .clone();
                    edge.ref_id = relation.relation_ref.clone();
                    edge.revision = 1;
                    edge.origin_ref = Some(request.frame_ref.clone());
                    edge.extensions.insert(RELATION.into(), encoded(&relation)?);
                    edge.extensions.insert(
                        "derived_from_relation".into(),
                        json!({"reference":edge_ref,"revision":prior.revision}),
                    );
            ...
            let anchor = WikiNode {
                profile: OKF_WIKI_PROFILE.into(),
                ref_id: anchor_ref.clone(),
                revision: 1,
                provenance: vec![],
                node_type: "Constellation".into(),
            ...
            if frame.revision != request.expected_revision {
                return Err(AikitError::new(
                    "knowledge.constellation_revision_conflict",
                    "construction changed; inspect and explicitly reconcile",
                ));
            }
            if frame.extensions.get("read_only") == Some(&Value::Bool(true))
                || frame.extensions.contains_key("shared_projection_ref")
            {
                return Err(err(
            ...
            if frame.constellations.len() != 1 {
                return Err(err(
                    "this operation requires one addressed constellation per native frame",
                ));
            }
        }
        for (index, change) in request.changes.iter().enumerate() {
            match change {
                ...
                Change::Create { .. } => {
                    return Err(err("create cannot replace an existing construction"))
                }
                Change::InquirySet { title, inquiry } => {
                    meta.title = title.clone();
                    meta.inquiry = inquiry.clone();
                }
                ...
                Change::MemberAdd { member } => {
                    ...
                    if frame.constellations[0].members.iter().any(|m| {
                        participation(m).is_ok_and(|p| {
                            p.participation_ref == member.participation.participation_ref
                        })
                    }) {
                        return Err(err("participation identity already exists"));
                    }
                    if member.subject_ref == frame.constellations[0].anchor_ref {
                        return Err(err("whole anchor is not an extra positional member"));
                    }
                    frame.constellations[0]
                        .members
                        .push(WikiConstellationMember {
                            ref_id: member.subject_ref.clone(),
                            position: None,
                            conjugate: false,
                            extensions: BTreeMap::from([(
                                PARTICIPATION.into(),
                                encoded(&member.participation)?,
                            )]),
                        });
                }
                ...
                Change::RoleSet {
                    participation_ref,
                    role_ref,
                } => revise_member(&mut frame, participation_ref, |p| {
                    p.role_ref = role_ref.clone()
                })?,
                Change::SourcesSet {
                    participation_ref,
                    sources,
                } => revise_member(&mut frame, participation_ref, |p| {
                    p.sources = sources.clone()
                })?,
                Change::RelationPut { relation: r } => {
                    ...
                    if !["directed", "undirected", "bidirectional"].contains(&r.direction.as_str())
                        || !["proposed", "asserted", "contested", "uncertain"]
                            .contains(&r.standing.as_str())
                    {
                        return Err(err("invalid relation direction/standing"));
                    }
                    selection_sources(&r.evidence)?;
                    if let Some(WikiObject::Edge(existing)) = doc.object(&r.relation_ref) {
                        if existing.origin_ref.as_ref() != Some(&request.frame_ref) {
                            return Err(err(
                                "a construction cannot rewrite a foreign native relation",
                            ));
                        }
                    }
                    ...
                    if !r.temporal.is_empty() {
                        write_facets_to_extensions(
                            &mut extensions,
                            &TechneFacets {
                    ...
                    let edge = WikiEdge {
                        profile: OKF_WIKI_PROFILE.into(),
                        ref_id: r.relation_ref.clone(),
                        revision: 1,
                        provenance: r.evidence.clone(),
                        from_ref: frame.constellations[0].members[a].ref_id.clone(),
                        to_ref: frame.constellations[0].members[b].ref_id.clone(),
                        relation: r.relation.clone(),
                        origin: WikiEdgeOrigin::Authored,
                        origin_ref: Some(request.frame_ref.clone()),
                        extensions,
                    };
                    native_put(doc, ledger, WikiObject::Edge(edge), r.expected_revision)?;
                    if !meta.relation_refs.contains(&r.relation_ref) {
                        meta.relation_refs.push(r.relation_ref.clone());
                    }
                }
                Change::RelationRetract {
                    relation_ref,
                    expected_revision,
                    reason,
                } => {
                    text(reason, "retraction reason", 4096)?;
                    let Some(WikiObject::Edge(existing)) = doc.object(relation_ref) else {
                        return Err(err("relation is absent"));
                    };
                    if existing.origin_ref.as_ref() != Some(&request.frame_ref) {
                        return Err(err(
                            "foreign relation cannot be retracted by this construction",
                        ));
                    }
                    ...
                    let details = edge
                        ...
                        .ok_or_else(|| err("missing native relation participation basis"))?;
                ...
                Change::AlternativeAdd { alternative } => {
                    check_reference(alternative, &request.frame_ref)?;
                    if !meta.alternatives.contains(alternative) {
                        meta.alternatives.push(alternative.clone());
                    }
                }
                Change::AlternativeRemove { whole_ref } => {
                    meta.alternatives.retain(|r| &r.whole_ref != whole_ref)
                }
                Change::CompositionAttach { composition } => {
                    if !["expression", "journey", "palace", "artifact"]
                        .contains(&composition.kind.as_str())
                    {
                        return Err(err("unsupported composition kind"));
                    }
                    ResourceRef::parse(composition.reference.as_str())?;
                    for reference in &composition.derivation_refs {
                        ResourceRef::parse(reference.as_str())?;
                    }
                    text(&composition.revision, "composition revision", 1024)?;
                    selection_sources(std::slice::from_ref(&composition.source))?;
                    if composition.derivation_refs.is_empty() {
                        return Err(err(
                            "returned composition/artifact must retain its derivation",
                        ));
                    }
                    if let Some(i) = meta
                        .compositions
                        .iter()
                        .position(|c| c.reference == composition.reference)
                    {
                        meta.compositions[i] = composition.clone();
                    } else {
        ...
        if meta.applied.len() >= LIMIT {
            return Err(err(
                "operation receipt budget reached; preserve history before continuing",
            ));
        }
        ...
        frame.constellations[0].returns = meta
            .compositions
            .iter()
            .map(|composition| WikiConstellationReturn {
                through_anchor_ref: through.clone(),
                ground_ref: composition.reference.clone(),
                ground_kind: Some("own".into()),
                extensions: BTreeMap::from([
                    ("derivation_refs".into(), json!(composition.derivation_refs)),
                    ("revision".into(), json!(composition.revision)),
                    ("kind".into(), json!(composition.kind)),
                ]),
            })
            .collect();
        frame.member_refs = members.into_iter().collect();
        let mut external: BTreeSet<_> = frame
            .member_refs
            .iter()
            .filter(|r| !doc.holds(r))
            .cloned()
            .collect();
        external.extend(
            meta.compositions
                .iter()
                .filter(|c| !doc.holds(&c.reference))
    ...
    let content = if outcome.changed {
        content
    } else {
        input.into()
    };
    ...
    Ok(Applied {
        ...
        content,
        objects_changed: outcome.touched.iter().map(|t| t.resource.clone()).collect(),
        warnings: outcome.warnings,
    })
}
fn validate_construction(doc: &WikiDocument, frame: &WikiFrame, meta: &Construction) -> Result<()> {
    text(&meta.title, "construction title", 1024)?;
    text(&meta.inquiry.question, "inquiry question", 16384)?;
    if meta.inquiry.purpose.len() > 16384 {
        return Err(err("purpose budget exceeded"));
    }
    let mut roles = BTreeSet::new();
    if let Some(form) = &meta.frame {
        text(&form.shape_ref, "shape ref", 1024)?;
        text(&form.contract_ref, "shape contract", 1024)?;
        if !["proposed", "established"].contains(&form.standing.as_str()) {
            return Err(err("frame must retain its proposed/established standing"));
        }
        if form.standing == "established" && form.provenance.is_empty() {
            return Err(err(
                "an established QL reading requires its warrant; proposed authorship does not",
            ));
        }
        selection_sources(&form.provenance)?;
        if form.roles.len() > LIMIT {
            return Err(err("role budget exceeded"));
        }
        for role in &form.roles {
            ResourceRef::parse(role.role_ref.as_str())?;
            if !roles.insert(role.role_ref.clone()) {
                return Err(err("duplicate frame role"));
            }
            text(&role.label, "role label", 1024)?;
            if role.address.is_null() {
                return Err(err("role lacks its QL address"));
            }
        }
    }
    let mut ids = BTreeSet::new();
    if frame.constellations[0].members.len() > LIMIT {
        return Err(err("membership budget exceeded"));
    }
    for member in &frame.constellations[0].members {
        let p = participation(member)?;
        if !ids.insert(p.participation_ref.clone()) {
            return Err(err("duplicate participation identity"));
        }
        if p.role_ref.as_ref().is_some_and(|r| !roles.contains(r)) {
            return Err(err("membership role is outside the authored frame"));
        }
        ...
        if let Some(nested) = &p.nested {
            ...
            if nested.kind == "constellation" {
                ...
                check_nested(doc, &nested.whole_ref, &frame.ref_id, &mut seen, 0)?;
            }
        }
    }
    Ok(())
...
fn check_nested(
    ...
    reference: &ResourceRef,
    target: &ResourceRef,
    seen: &mut BTreeSet<ResourceRef>,
    depth: usize,
) -> Result<()> {
    if reference == target {
        return Err(err("nested whole cycle"));
    }
    if depth >= 64 {
        return Err(err("nested disclosure depth exceeded"));
    }
    ...
    if let Some(WikiObject::Frame(frame)) = doc.object(reference) {
        for constellation in &frame.constellations {
            for member in &constellation.members {
                if let Ok(p) = participation(member) {
                    if let Some(n) = p.nested {
                        if n.kind == "constellation" {
                            check_nested(doc, &n.whole_ref, target, seen, depth + 1)?;
                        }
                    }
                }
            }
