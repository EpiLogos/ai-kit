        ...
        impl $name {
            pub fn new(value: impl Into<String>) -> Result<Self> {
                ...
                if value.trim().is_empty() {
                    return Err(AikitError::InvalidInput(format!(
...
impl SecretRequirement {
    pub fn validate(&self) -> Result<()> {
        if self.consumer_ref.trim().is_empty() {
            return Err(AikitError::InvalidInput(
                "secret requirement consumer_ref must not be empty".into(),
            ));
        }
        if self.purpose.trim().is_empty() {
            return Err(AikitError::InvalidInput(
                "secret requirement purpose must not be empty".into(),
            ));
        }
        if self.permitted_materialisation.is_empty() {
            return Err(AikitError::InvalidInput(
                "secret requirement must permit at least one materialisation class".into(),
            ));
        }
        Ok(())
    }
...
impl SecretProviderDescriptor {
    pub fn validate(&self) -> Result<()> {
        ...
        if self.supported_materialisation.is_empty() {
            return Err(AikitError::InvalidInput(
                "secret provider must advertise at least one materialisation class".into(),
            ));
        }
        Ok(())
    }
...
pub fn resolve_credential(request: CredentialResolutionRequest) -> Result<CredentialResolution> {
    ...
    for provider in request.providers {
        ...
        let explanation = ProviderResolutionExplanation {
...
/// Safe provider-binding state. `CredentialRef` remains stable when this changes.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
...
mod tests {
    ...
    fn rotation_changes_provider_revision_not_credential_ref() {
        let credential_ref = credential("credential:openai/research");
        let v1 = CredentialBindingState {
        ...
        let v2 = CredentialBindingState {
            revision_or_lease_class: Some("revision:v2".into()),
            ..v1.clone()
        };

        assert_eq!(v1.credential_ref, v2.credential_ref);
        assert_ne!(v1.revision_or_lease_class, v2.revision_or_lease_class);
    }

    #[test]
    fn provider_replacement_changes_binding_provenance_not_credential_ref() {
        let credential_ref = credential("credential:openai/research");
