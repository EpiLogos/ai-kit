...
pub fn candidates_from_routes(
    ...
    routes
        ...
        .map(|route| {
            ...
            candidate
                .provenance
                .extend(route.provenance.iter().cloned());
            candidate
        })
        .collect()
}
...
pub fn select_model(
    ...
    let viable: Vec<ModelRoute> = match pinned_provider {
        Some(provider) => routes
            .viable_pinned(provider)
            .into_iter()
            .cloned()
            .collect(),
    ...
    let mut ranked: Vec<&ModelRosterEntry> = roster
        ...
        .filter(|entry| {
            entry.model == routes.model
                ...
                && viable.iter().any(|route| route.provider == entry.provider)
        })
        .collect();
...
mod tests {
    ...
    fn observed_route(
        ...
        provider: &str,
        native: &str,
        kind: crate::resource::ModelRouteKind,
    ) -> ModelRoute {
        ModelRoute {
            model: r(model),
            provider: p(provider),
    ...
    fn selection_orders_routes_by_rank_without_deleting_the_others() {
        ...
        let selection = select_model(&roster, &set, None).unwrap();
        assert_eq!(
            selection.viable_routes[0].provider.as_str(),
            "provider:ninerouter"
        );
    ...
    fn losing_the_selected_route_re_resolves_to_the_survivor_under_the_same_model_ref() {
        ...
        assert_eq!(
            ...
            "the ModelRef survives route loss"
        );
        assert_eq!(after.viable_routes.len(), 1);
        assert_eq!(
            after.viable_routes[0].provider.as_str(),
            "provider:ninerouter"
        );
    }
