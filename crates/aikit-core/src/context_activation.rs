...
pub fn attach_context_activations(
    ...
    resolution.context_activations.sort_by(|left, right| {
        left.source
            .cmp(&right.source)
            .then_with(|| left.target.cmp(&right.target))
            .then_with(|| activation_mode_rank(left.mode).cmp(&activation_mode_rank(right.mode)))
            .then_with(|| left.loader.cmp(&right.loader))
    });
    Ok(())
}
...
fn activation_mode_rank(mode: ContextActivationMode) -> u8 {
    match mode {
        ContextActivationMode::AikitSelected => 0,
        ContextActivationMode::Retrieved => 1,
        ContextActivationMode::Projected => 2,
        ContextActivationMode::HarnessNativeAutoLoaded => 3,
        ContextActivationMode::TargetNativeOther => 4,
    }
}
