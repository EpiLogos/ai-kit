...
impl HistoryEvidence {
    pub fn matches(&self, resource: &ResourceRef) -> bool {
        &self.subject == resource || self.canonical_refs.iter().any(|candidate| candidate == resource)
    }
}
...
pub fn explain_resource_evidence(explanation: &ResourceExplanation) -> ExplainEvidence {
    ...
    for source in &explanation.sources {
        facts.push(ExplainFact {
            ...
            canonical_refs: ResourceRef::parse(source.source.as_str()).into_iter().collect(),
    ...
    if let Some(preference) = &explanation.preference {
        facts.push(ExplainFact {
            ...
            authority: Some(SourceAuthority::Authored),
            summary: format!("authored rank {} from {}", preference.rank, preference.source),
...
pub fn familiarity_history_evidence(observation: &FamiliarityObservation) -> HistoryEvidence {
    ...
    details.insert("observedAtMs".into(), observation.observed_at_ms.to_string());
...
pub fn harness_composition_history_evidence(
    ...
    details.insert("retracted".into(), diff.retracted_components.len().to_string());
    details.insert("reboundContracts".into(), diff.rebound_contracts.len().to_string());
    ...
    HistoryEvidence {
        ...
        summary: format!(
            ...
            if diff.mounted_components.len() == 1 { "" } else { "s" },
            diff.retracted_components.len(),
            if diff.retracted_components.len() == 1 { "" } else { "s" },
            diff.rebound_contracts.len(),
            if diff.rebound_contracts.len() == 1 { "" } else { "s" },
        ),
        canonical_refs: canonical_refs.into_iter().collect(),
        provenance: Vec::new(),
...
mod tests {
    ...
    fn knowledge_route_history_is_learned_navigation_not_relation_truth() {
        ...
        assert_eq!(history.recoverability, HistoryRecoverability::ReplayNavigation);
        assert!(history.canonical_refs.contains(&r("knowledge-route/spec-to-code")));
        assert!(history.canonical_refs.contains(&r("knowledge-node/spec")));
        assert!(history.canonical_refs.contains(&r("code-reference/handler")));
