...
mod tests {
    ...
    fn etymological_shape_frame(flow_ref: &ResourceRef) -> WikiObject {
    ...
    fn etymological_flow_enters_shape_generates_whole_and_returns_to_exact_source_basis() {
        ...
        assert_eq!(
            preflight.flow.standing.binding.flow_ref,
            provider.flow.flow_ref
        );
        assert_eq!(
        ...
        assert!(
            living.ql.is_none(),
            "portable shape operation needs no live QL-MEF provider"
        );

        let original_body = provider.body.clone();
        let mut executor = EtymologicalShapedFlowExecutor::default();
        let outcome =
            explicit_ql_shaped_flow_contemplate(&request, DEFAULT_QL_SHAPE_BUDGET, &mut executor)
                .unwrap();
        assert_eq!(executor.calls, 1);
        assert!(outcome.flow.flow_mutations.is_empty());
        assert_eq!(
            provider.body, original_body,
            "generated Agent knowledge does not rewrite authored Flow source"
        );
        assert!(outcome
            .flow
            .living
        ...
        assert_eq!(
            flow_basis.source_revision.as_ref(),
            Some(&provider.flow.revision)
        );
