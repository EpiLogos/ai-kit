...
patch(search, "use crate::resolve::ResolvedView;\n",
...
patch(fam, "use crate::resource::ResourceRef;\n",
...
'''    pub fn resolve_path(\n        observation_id: impl Into<String>,\n        route: ResourceRef,\n        destination: ResourceRef,\n        steps: Vec<RouteStepEvidence>,\n        operative: OperativePathEvidence,\n        context: FamiliarityContext,\n        observed_at_ms: u64,\n    ) -> Result<Self> {\n        if steps.is_empty() {\n            return Err(AikitError::new("familiarity.empty_resolve_path", "a ResolvePath observation needs route steps"));\n        }\n        if steps.last().is_none_or(|step| step.resource != destination) {\n            return Err(AikitError::new("familiarity.resolve_path_destination_mismatch", "the final ResolvePath step must be the destination"));\n        }\n        Ok(Self {\n            observation_id: observation_id.into(), destination, context,\n            use_kind: FamiliarityUse::ResolvePath { route, steps, operative },\n            source_surface: None, source_action: None, observed_at_ms, fitness: None,\n        })\n    }\n\n    #[must_use]\n    pub fn from_surface(mut self, surface: ResourceRef) -> Self {\n''')
patch(fam,
...
pub fn actor_world_disclosure(bootstrap: &ActorBootstrap) -> ResolveExpression {
    ...
    fn clause(h: AddressHorizon, r: ResourceRef) -> ResolveExpression {
        ResolveExpression::Unary {
            op: RelationOp::Affirm,
            expression: Box::new(ResolveExpression::horizon(h, ResolveExpression::subject(r.to_string()))),
        }
    }
    let mut clauses = Vec::new();
    clauses.extend(bootstrap.context_sources.examples.iter().cloned().map(|r| clause(AddressHorizon::H0, r)));
    clauses.extend([actual(&bootstrap.host), actual(&bootstrap.harness), actual(&bootstrap.model)].into_iter().flatten().map(|r| clause(AddressHorizon::H1, r)));
    clauses.extend([actual(&bootstrap.agent), actual(&bootstrap.agency)].into_iter().flatten().map(|r| clause(AddressHorizon::H2, r)));
    clauses.extend([actual(&bootstrap.harness), actual(&bootstrap.model)].into_iter().flatten().map(|r| clause(AddressHorizon::H3, r)));
    if let Ok(project) = ResourceRef::parse(bootstrap.project.project.as_str()) { clauses.push(clause(AddressHorizon::H4, project)); }
    if let Some(run) = bootstrap.run.clone() { clauses.push(clause(AddressHorizon::H4, run)); }
    clauses.extend([actual(&bootstrap.agent), actual(&bootstrap.host)].into_iter().flatten().map(|r| clause(AddressHorizon::H4, r)));
    clauses.extend(bootstrap.capabilities.examples.iter().chain(&bootstrap.actions.examples).cloned().map(|r| clause(AddressHorizon::H5, r)));
    let expression = clauses.into_iter().reduce(|left, right| ResolveExpression::Binary {
        op: RelationOp::Contextualise, left: Box::new(left), right: Box::new(right),
    }).unwrap_or_else(|| ResolveExpression::ordinary_search(""));
    ResolveExpression::Frame { expression: Box::new(expression) }
}
'''
p.write_text(s)
