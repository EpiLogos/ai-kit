...
| Harness profiles | Declarative `aikit.harness-profile/v1` documents (detection, projection and ownership posture per layer: skills, guidance, hooks, tools, models, sessions), a `tool-protocol` capsule kind for MCP servers as capability sources, and one layer merge engine behind parity-tested grammars. Embedded profiles ship with the binary; external documents are admitted through the public intake — `aikit harness-profile validate|show|register|list` (grammar: `docs/HARNESS-PROFILE-AUTHORING.md`) — and load from `$AIKIT_HOME/harness-profiles/` without overriding embedded slugs. Design: `docs/plans/2026-09-16-harness-profile-design.md`. |
